...
class TestRebuildReplicaRoutingChaos:
    ...
    def test_search_skips_failed_rebuild_replica(self):
        """A terminally-failed rebuild must take the replica off reads: the
        replica is marked ReplicasRebuildFailed and searches stay available on
        the surviving replicas. The failed replica is killed to drive the
        rebuild to a terminal 'failed' state; the marker is set by the master
        and persists in the partition record while the victim PS is down."""
        _ensure_all_ps_alive()
