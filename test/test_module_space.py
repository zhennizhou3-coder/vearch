...
class TestSpaceCreate:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [35, "training_threshold below ncentroids*39", "IVFPQ"],
        ],
    )
    def test_vearch_space_create_badcase(self, wrong_index, wrong_type, index_type):
        ...
        if wrong_index == 34:
            qb = 9
        if wrong_index == 35:
            # >= min_training_threshold (256) but < ncentroids * 39,
            # so it clears the old floor yet fails the per-centroid minimum.
            ncentroids = 1000
