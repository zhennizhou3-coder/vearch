...
func (ca *clusterAPI) rebuildIndex(c *gin.Context) {
	...
	// partition_id and index_name both scope the rebuild to a specific space,
	// so either one requires both dbName and spaceName.
	if (rebuildReq.PartitionId != 0 || rebuildReq.IndexName != "") && (dbName == "" || spaceName == "") {
		log.Warn("rebuildIndex: partition_id/index_name require both dbName and spaceName (partition_id=%d, index_name=%s)",
			rebuildReq.PartitionId, rebuildReq.IndexName)
		httpCode = response.New(c).JsonError(errors.NewErrBadRequest(
			fmt.Errorf("partition_id and index_name require both dbName and spaceName to be specified")))
		return
	}
	type target struct {
		db    string
