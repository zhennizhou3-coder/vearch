	...
	if len(tempIndex.Params) != 0 {
		...
		if tempIndex.Type == "HNSW" {
			...
			if indexParams.TrainingThreshold != 0 {
				...
				if indexParams.Ncentroids > 0 {
					clampLow := indexParams.Ncentroids * DefaultMinPointsPerCentroid
					if indexParams.TrainingThreshold < clampLow {
						return vearchpb.NewError(vearchpb.ErrorEnum_PARAM_ERROR, fmt.Errorf(tempIndex.Type+" training_threshold:[%d] is below the engine's auto-clamp lower bound ncentroids*%d=[%d]; either raise training_threshold to >=%d or lower ncentroids", indexParams.TrainingThreshold, DefaultMinPointsPerCentroid, clampLow, clampLow))
					}
				}
			} else {
				if indexParams.Ncentroids != 0 && indexParams.Ncentroids*DefaultMinPointsPerCentroid < MinTrainingThreshold {
			...
			if indexParams.Nprobe != 0 {
				if indexParams.Nprobe > indexParams.Ncentroids {
					return vearchpb.NewError(vearchpb.ErrorEnum_PARAM_ERROR, fmt.Errorf(tempIndex.Type+" nprobe:[%d] should less than ncentroids:[%d]", indexParams.Nprobe, indexParams.Ncentroids))
				}
			} else if indexParams.Ncentroids > 0 && indexParams.Ncentroids < DefaultNprobe {
				// Default nprobe would exceed ncentroids; reject early with a clear error.
				return vearchpb.NewError(vearchpb.ErrorEnum_PARAM_ERROR, fmt.Errorf(tempIndex.Type+" ncentroids:[%d] < default nprobe:[%d]; set nprobe explicitly (recommended: nprobe<=ncentroids)", indexParams.Ncentroids, DefaultNprobe))
			}
		}
	}
