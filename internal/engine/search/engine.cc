...
int Engine::RebuildIndex(const std::string &index_name,
  ...
  if (vec_manager_->RawVectors().count(field_name) == 0) {
    LOG(INFO) << space_name_ << " RebuildIndex: field " << field_name
              << " is not a vector field, nothing to rebuild";
    return 0;
  }

  // Stop the background indexing thread before mutating the index (master's
  // shared helper; replaces the old PrepareRebuild gate). Done only after the
  // early-return branches above (read-only describe / non-vector field), so a
  ...
  // Restart the background indexing thread that StopIndexingThread stopped
  // above, so subsequent realtime writes to this partition keep flowing into
  // the index. StopIndexingThread left indexing_state_ at IDLE; BuildIndex
  // CAS-es IDLE->STARTING and spawns the loop. Mirrors AddVectorFieldIndex: a
  // BuildIndex miss is a best-effort trigger miss (the next indexing pass
  // retries), not a structural failure, so it is logged rather than fatal.
  if (refresh_interval_ >= 0 &&
    ...
    if (ret) {
      LOG(ERROR) << space_name_
                 << " RebuildIndex BuildIndex (restart indexing) failed, "
                    "ret: " << ret
                 << "; index is rebuilt, indexing will resume on next pass";
    }
  }
  ...
  if (!compact_status.ok()) {
    LOG(ERROR) << space_name_
               << " RebuildIndex compact vector error: "
               << compact_status.ToString();
    return -1;
  }

  LOG(INFO) << space_name_ << " RebuildIndex for " << index_name
...
void Engine::AddVectorFieldIndex(const std::string &index_name,
  ...
  vec_manager_->AddIndexTypeAndParam(index_name, field_name, indexType,
                                     indexParam);
