...
// align to NPU size
const int kMaxDeviceBatchAlign = 64;
const int kMaxBatchQueryVectors = 512;
const double kBatchLingerMs = 3.0;
...
inline int DeviceAlignedQueryTarget(int num_queries) {
  if (num_queries >= kMaxDeviceBatchAlign) {
    int target = (num_queries / kMaxDeviceBatchAlign) * kMaxDeviceBatchAlign;
    return target > kMaxBatchQueryVectors ? kMaxBatchQueryVectors : target;
  }
  return FloorPow2(num_queries);
}
}
...
int GammaIVFRABITQNPUIndex::NPUSearchThread() {
  ...
  // Items dequeued last round but not submitted (they overflowed the
  // device-aligned target); they seed the next round instead of being requeued.
  NPUSearchItem *carry[kMaxBatchItems];
  int carry_count = 0;
  ...
  while (!b_exited_) {
    ...
    if (carry_count > 0) {
      std::memcpy(items, carry, carry_count * sizeof(NPUSearchItem *));
      item_count = carry_count;
      carry_count = 0;
    } else {
      item_count = search_queue_.wait_dequeue_bulk_timed(items, kMaxBatchItems, 100);
    }
    if (item_count == 0) continue;

    int vector_count = 0;
    for (int i = 0; i < item_count; ++i) vector_count += items[i]->n_;

    if (vector_count < kMaxBatchQueryVectors && item_count < kMaxBatchItems && !b_exited_) {
      double deadline = utils::getmillisecs() + kBatchLingerMs;
      while (vector_count < kMaxBatchQueryVectors && item_count < kMaxBatchItems &&
             !b_exited_) {
        double remain_ms = deadline - utils::getmillisecs();
        if (remain_ms <= 0) break;
        int dequeue_limit = kMaxBatchItems - item_count;
        int got = search_queue_.wait_dequeue_bulk_timed(
            items + item_count, static_cast<size_t>(dequeue_limit),
            static_cast<std::int64_t>(remain_ms * 1000) /*us*/);
        if (got <= 0) break;
        for (int i = item_count; i < item_count + got; ++i)
          vector_count += items[i]->n_;
        item_count += got;
      }
    }

    int vector_target = DeviceAlignedQueryTarget(vector_count);
    int submit_items = 0, submit_vectors = 0;
    for (int i = 0; i < item_count; ++i) {
      if (submit_vectors + items[i]->n_ > vector_target) break;
      submit_vectors += items[i]->n_;
      submit_items = i + 1;
    }
    if (submit_items == 0) submit_items = 1;
    carry_count = item_count - submit_items;
    if (carry_count > 0) {
      std::memcpy(carry, items + submit_items,
                  carry_count * sizeof(NPUSearchItem *));
    }
    int size = submit_items;
  ...
  for (int i = 0; i < carry_count; ++i) {
    if (carry[i]) carry[i]->NotifyFailure(NPUSearchItem::INDEX_UNAVAILABLE);
  }
  carry_count = 0;
