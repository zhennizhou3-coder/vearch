...
command: 备份命令，指定为 backup 或者 restore
...
::
  ...
  --data '{
      "command": "restore",
      "version_id": "202601191425",
      "source_cluster_name": "your-cluster-name",
      "with_schema": true,
      "s3_param": {
          "access_key": "USER_ACCESS_KEY",
          "bucket_name": "USER_BUCKET_NAME",
...
version_id: 指定恢复的version_id

source_cluster_name: 指定恢复的数据源，实现跨集群恢复
...
::
  ...
  curl --location --request DELETE 'http://${VEARCH_URL}/backup/dbs/${DB_NAME}/spaces/${SPACE_NAME}/versions/${VERSION_ID}/direct/' \
  --data '{
      "s3_param": {
          "access_key": "USER_ACCESS_KEY",
          "bucket_name": "USER_BUCKET_NAME",
...
curl --location 'http://${VEARCH_URL}/backup/dbs/${DB_NAME}/spaces/${SPACE_NAME}/progress' \
...
status: 备份状态, completed/running

total_tasks: 分片备份任务个数

completed_tasks: 已完成的任务格式

success_ratio: 当前备份成功的分片任务比例
...
curl --location 'http://${VEARCH_URL}/restore/dbs/${DB_NAME}/spaces/${SPACE_NAME}/progress' \
...
status: 恢复状态, completed/running

total_tasks: 分片恢复任务个数

completed_tasks: 已完成的任务格式

success_ratio: 当前恢复成功的分片任务比例
