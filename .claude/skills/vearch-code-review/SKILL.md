...
description: Use when reviewing Vearch PRs/diffs or preparing code for review. Anchors checks to Vearch-specific invariants — raft write path, anti-affinity, generated `.pb.go`, cgo `-tags="vector"`, etcd key access, replica change ordering — in addition to general correctness, testing, and style. Writes the review as a Chinese Markdown file under `.claude/code-reviews/`.
---
...
**REQUIRED SUB-SKILL:** Use `vearch-review-output` for where and how to write the report — output language, target directory (`.claude/code-reviews/`), filename convention, and report header. 正文按下面的清单逐节给出，发现项标注严重级别（阻断 / 高 / 中 / 低）并附 `file:line` 锚点。
...
- [ ] Deterministic engine units (helpers, bit masks, kill/status maps, id encoding) have a self-contained gtest under `internal/engine/tests/` — each `*.cc` there is auto-globbed into its own executable, so write it to need no dataset args. Add this coverage even when the end-to-end behavior it supports (e.g. mid-search cancellation) is timing-dependent and only reachable through `test/` integration tests.
...
- [ ] Comments explain non-obvious WHY (raft constraints, anti-affinity reasons, perf workarounds); they do not narrate WHAT the code already says. After extracting a well-named helper, redundant per-call-site comments are removed.
- [ ] Implicit numeric/bit invariants (power-of-two stride+mask, alignment, buffer sizing) are enforced at compile time (template parameter + `static_assert`) rather than left as a convention a later edit can silently break.
